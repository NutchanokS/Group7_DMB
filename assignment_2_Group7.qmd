...
To robustly evaluate the three restroom configurations, this study employs a two-stage analytical framework as seen in @fig-two-stage. We transition from a highly structured, static queueing model to a dynamic, stochastic simulation designed to stress-test the configurations under realistic, peak-demand intermission pressures.

![The Two-Stage Analytical Framework](images/paste-9.png){#fig-two-stage}

The primary measures of success for this analysis are: (1) Totalitarian Satisfaction Score (Tr), measuring aggregate patron satisfaction across all groups; (2) Rawlsian Satisfaction Score (Mr), measuring the satisfaction of the worst-off patron group; and (3) Incompletion Rate, measuring the proportion of patrons unable to complete their restroom visit within the intermission window.
...
## 2.1 Case Parameter and Assumptions

All model inputs are drawn from historical operational data observed at the
venue. The following tables consolidate the key parameters and configurations
used across both stages of the analysis, serving as the reference point
for all subsequent modelling.

### 2.1.1 Input Parameters

**Table 1 — Observed Arrival and Service Time Parameters**

| Parameter | Women | Men |
|:---|:---:|:---:|
| Total arrival rate (Poisson, patrons/min) | 3.2 | 3.2 |
| Class 2 probability | 5% | 5% |
| Class 1 mean service time (min) | 1.67 | 0.833 |
| Class 1 service time SD (min) | 1.20 | 0.700 |
| Class 2 mean service time (min) | 5.20 | 5.200 |
| Class 2 service time SD (min) | 3.20 | 3.200 |

*The combined arrival rate across both genders is λ = 6.4 patrons/min,
yielding a total patron volume of N = 160 over the 25-minute intermission.*

**Table 2 - Facility Preference Distribution**

+------------------------+----------------------+-----------------------+
| Preference Type        | Men                  | Women                 |
+:=======================+:====================:+:=====================:+
| Gender-segregated (GS) | 70%                  | 70%                   |
+------------------------+----------------------+-----------------------+
| No preference (NP)     | 21%                  | 21%                   |
+------------------------+----------------------+-----------------------+
| Unisex (UX)            | 9%                   | 9%                    |
+------------------------+----------------------+-----------------------+
...
*Scores are mapped using the bracket of the patron's actual
wait time.*

**Table 4 — Configuration Fixture Allocation**

+----------------+------------------+------------------+------------------+
| Restroom Type  | Configuration 1\ | Configuration 2\ | Configuration 3\ |
|                | Status Quo       | All Unisex       | Hybrid           |
+:===============+:================:+:================:+:================:+
...
### 2.1.2 General Modelling Assumptions
...
-   No-preference users are assumed to split equally (50:50) between gender-segregated and unisex facilities where available. In the simulation scenarios.

- The intermission window is fixed at 25 minutes. Any patron whose
  combined wait time and service time exceeds this threshold is recorded
  as incomplete, representing a patron unable to return to their
  seat before the next act begins.

-   Steady-state conditions are assumed for the baseline analytical model.

## 2.2 Stage 1: Steady-State Modeling

We begin by establishing a theoretical baseline using analytical queueing equations. To set a clean, idealised benchmark, Stage 1 assumes a perfectly balanced 50:50 gender demographic split and a Poisson arrival rate throughout the intermission, meaning patrons arrive at a constant average rate (λ) over the entire break with no sudden rushes.
...
+-----------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Outputs**     | Expected waiting time in queue (Wq), server utilisation (ρ), Totalitarian Satisfaction Score (Tr), and Rawlsian Satisfaction Score (Mr) for each configuration                                                                    |
+-----------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Assumptions** | Poisson arrivals, infinite queue capacity, first-come-first-served, independence between segregated queues, and Gamma-distributed service times parameterised from the historical case data, 50:50 split on no-preference patrons |
+-----------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+

To model this baseline with mathematical rigour, we leverage the historical data provided in the case description, which includes the empirical mean service time and standard deviation (σ) for different restroom activities. With these explicit parameters, we can calculate the exact service time coefficient of variation (CV = σ/Mean). Because the empirical service time data yields a coefficient of variation CV ≠ 1, the M/G/k model is adopted as the most appropriate baseline, with the Allen-Cunneen approximation used to compute expected waiting times. In this model, we choose Gamma distributions to represent service times, as they can flexibly capture the observed variability while ensuring non-negativity.
...
## 2.3 Stage 2: Stress-Test Scenario Design
...
+-----------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Assumptions** | Total patron volume is fixed at N = 160 across all runs to ensure comparability, Poisson arrivals, infinite queue capacity, patrons do not leave the queue once joined, first-come-first-served, independence between segregated queues, and Gamma-distributed service times parameterised from the historical case data, no-preference patrons split 50:50 between dedicated and unisex facilities, and each replication uses a different random seed |
+-----------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
For the Late Burst scenario, these phases are simply reversed: the low-intensity tail rate ( $λ_{tail}$ = 1.6) applies to the first 20 minutes, and the high-intensity burst rate ($λ_{burst}$ = 25.6) applies to the final 5 minutes. The total patron volume remains N = 160 in both cases.
...
-   **Scenario 1: Women Dominated with an Early Burst**

*Parameters:* 80% Women / 20% Men audience; 80% of arrivals occur in the first 5 minutes.

\[placeholder for illustration of Scenario 1 arrival profile\]

-   **Scenario 2: Women Dominated with a Late Burst**

*Parameters:* 80% Women / 20% Men audience; 80% of arrivals occur in the final 5 minutes. \[placeholder for illustration of Scenario 2 arrival profile\]

-   **Scenario 3: Men Dominated with an Early Burst**

*Parameters:* 20% Women / 80% Men audience; 80% of arrivals occur in the first 5 minutes. \[placeholder for illustration of Scenario 3 arrival profile\]

-   **Scenario 4: Men Dominated with a Late Burst**

*Parameters:* 20% Women / 80% Men audience; 80% of arrivals occur in the final 5 minutes. \[placeholder for illustration of Scenario 4 arrival profile\]

To ensure statistical reliability and smooth out stochastic variation, each of the 12 experimental combinations (3 Configurations in each of 4 Scenarios) is executed across 500 replications with different random seeds, allowing us to compute expected averages and 95% confidence intervals for all performance metrics.

## 2.4 Stage 2: Discrete-Event Simulation (DES) Architecture
...
### 2.4.1 The Modular Process Approach
...
-   Arrival Time Generation: While Stage 1 assumes a single Poisson arrival rate, Stage 2 applies the two-phase arrival structure derived in Section 2.2, using $λ_{burst}$ and $λ_{tail}$ to generate the appropriate number of arrivals across the burst and tail phases for each scenario.

-   Service Time and Attribute Assignment: For each of the 160 patrons, the DGP assigns a demographic gender, an operational class (defining their restroom needs), and an empirical facility privacy preference. Based on these variables, a unique service time ($ST_i$) is sampled from a Gamma distribution.
...
This part takes the pre-generated pool of 160 patrons and processes them in event-driven sequence over the 25-minute timeline ($T = [0, 25]$ minutes). It tracks server availability using scoreboard vectors and manages queues under a first-come-first-served discipline based on the following behavioral routing rules:

-   Class-Specific Routing: Class 1 male patrons (using urinals) are routed strictly to the 10 dedicated male urinals. The only exception is Class 1 males with a unisex preference, who may route to a unisex stall if it is available.

-   Preference Routing: Patrons with strong segregation preferences will only queue for their designated gender-segregated facilities.

-   No-Preference Routing: Patrons with no strong facility preference will choose between dedicated gender spaces and unisex spaces based on a 50:50 distribution rule.
...
-   Incompletion Rate: The proportion of patrons that can not complete their restroom visit within 25-minute compared to the total patrons.
...
### 2.4.2 Configuration Differences and Design Loops
...
\[xxx-placeholder for visual flowchart of the configuration 3 routing logic-xxx –\> 50:50 no pereferenc split in DGPx\] \[xxx-placeholder for visual flowchart of the configuration 3 simulation-xxx\]

### 2.4.3 Combining the DGP and Routing Logic: The Execution Matrix
...
+-------------------------------------------------------+-------------------------------------------+-------------------------------------------+-------------------------------------+
| DGP Function                                          | RL - Configuration 1: `status_quo_loop()` | RL - Configuration 2: `all_unisex_loop()` | RL Configuration 3: `hybrid_loop()` |
+=======================================================+===========================================+===========================================+=====================================+
| **DGP / Scenario 1:** `dgp_female_dominated_early()`  | Run 1\                                    | Run 2                                     | Run 3                               |
|                                                       | \                                         |                                           |                                     |
+-------------------------------------------------------+-------------------------------------------+-------------------------------------------+-------------------------------------+
| **DGP 2 / Scenario 2:** `dgp_female_dominated_late()` | Run 4                                     | Run 5                                     | Run 6                               |
+-------------------------------------------------------+-------------------------------------------+-------------------------------------------+-------------------------------------+
| **DGP 3 / Scenario 3:** `dgp_male_dominated_early()`  | Run 7                                     | Run 8                                     | Run 9                               |
+-------------------------------------------------------+-------------------------------------------+-------------------------------------------+-------------------------------------+
| **DGP 4 / Scenario 4:** `dgp_male_dominated_late()`   | Run 10                                    | Run 11                                    | Run 12                              |
+-------------------------------------------------------+-------------------------------------------+-------------------------------------------+-------------------------------------+

Based on this execution matrix, with 4 DGP scenarios and 3 routing configurationsm we run 12 simulation in total, each repeated across 500 replications. The full code is provided in the Appendix, with key snippets highlighted in the main report to illustrate how the modular architecture is implemented.

## 2.5 Social Welfare Framework (The Fairness Metrics)
...
### 2.5.1 The 25-Minute Threshold and Incompletion Rate (Supplementary Metric)

While our primary objectives are maximizing overall crowd satisfaction and protecting minority demographics, we also capture a critical operational risk metric: incompletion rate.
...
### 2.5.2 Totalitarian Satisfaction: Overall Patron Welfare ($T_r$)
...
### 2.5.3 Rawlsian Fairness: Minimum Group Satisfaction ($M_r$)
...
For the baseline analysis, we apply the M/G/k queueing model to calculate expected waiting times and server utilisation for each line in the three configurations. These performance metrics are then translated into Totalitarian and Rawlsian satisfaction scores using the user preference matrix.

### 3.1.1 Configuration 1: Status Quo
...
### 3.1.2 Configuration 2: All Unisex
...
### 3.1.3 Configuration 3: Hybrid Model
...
### 3.1.4 Comparative Equity Analysis
...
|                       |                               |                   |              |
|                       | Status Quo / No Unisex Stalls | All Unisex Stalls | Hybrid Model |
+:=====================:+:=============================:+:=================:+:============:+
...
While the baseline results establish a clear ranking under idealised conditions, they also confirm that all three configurations distribute the same 40-unit capacity differently yet remain theoretically sufficient to serve all patrons within the intermission window. However, real concert hall intermissions rarely conform to steady-state assumptions, as patrons tend to surge toward restrooms immediately after the performance pauses or rush back just before it resumes. The following section examines whether this capacity sufficiency and equity ranking hold under these more realistic demand patterns.
...
The simulation results are interpreted using both operational and equity metrics. Operational performance is described using mean waiting time, mean system time, and the incompletion rate by the end of the 25-minute intermission. Mean waiting time captures queueing delay, while mean system time includes both waiting and service time.

Equity performance is evaluated using the same preference-based score table introduced in Section 2.1. The Totalitarian score represents the average user satisfaction score across all simulated users. The Rawlsian score represents the lowest average score among stakeholder groups, highlighting the experience of the worst-served group.

The ranking columns in the summary tables compare configurations within each scenario. Operational ranks are based on lower waiting time or lower incompletion rate, while equity ranks are based on higher Totalitarian and Rawlsian scores.
...
### 3.3.1 Facility-level queueing performance by scenarioxxx\[config performance\]
...
### 3.3.2 Operational performance across scenarios
...
### 3.3.3 Equity performance across scenarios
...
### 3.3.4 Overall comparison and ranking
...
# 4. Discussion of results and business recommendation

### 4.1 Key findings from the results

The overall results compare the three configurations across the baseline setting and four surge-demand simulation scenarios. The findings show a clear trade-off between queueing efficiency and preference-based fairness.

Across the surge-demand scenarios, the All Unisex configuration gives the strongest waiting-time performance. This is because all 40 restroom units are pooled into one shared system, allowing capacity to be used more flexibly by all users. This is especially clear in the early surge scenarios, where All Unisex records the lowest mean waiting time.

However, the Hybrid Model performs best on the equity-based measures. Across the baseline and all four stress-test scenarios, the Hybrid Model ranks first for both Totalitarian and Rawlsian scores. This means it provides the strongest average preference-based experience while also protecting the worst-off stakeholder group.

The results also show that late-arrival surge scenarios are more operationally difficult than early-arrival scenarios. When most users arrive in the last 5 minutes, a higher proportion of users are not completed before the 25-minute intermission ends. This suggests that demand timing is an important operational risk for management.

Overall, the results suggest the same decision direction across scenarios: All Unisex is the strongest option for pure queueing efficiency, Status Quo is the weakest option for inclusivity, and the Hybrid Model is the most balanced option when fairness, inclusivity, and stakeholder preference are included.
...
To make the recommendation more transparent, each configuration is assessed using a clear scope of consideration. A design should not be recommended only because it has the lowest average waiting time. It should also be considered in terms of fairness, inclusivity, robustness, and practicality.

In this report, the main decision considerations are:

+--------------+--------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|              | Criterion :============= Efficiency                                                              | Scope of consideration :=================================================================================================+ Overall average waiting time should remain low \| |
+--------------+--------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
This framework is used to avoid selecting a configuration based on one strong metric only. Since this report follows Direction A, fairness and stakeholder disparity are given higher priority than waiting-time efficiency alone. Therefore, the final decision should balance queueing performance with inclusive access and preference-based fairness.
...
The simulation specification is realistic because concert intermissions do not usually produce smooth arrivals. Many patrons may leave their seats immediately when intermission begins, while others may delay restroom use until closer to the end of the break. Therefore, modelling both first-5-minute and last-5-minute surge scenarios reflects plausible high-pressure demand patterns.

The gender-dominant burst scenarios are also reasonable because different event types may attract different audience mixes. Since men and women have different service-time distributions in the case data, changing the gender composition of the surge group changes the overall service-time distribution and tests whether each configuration remains robust under different stakeholder mixes.
...
Implementing the Hybrid Model may create some practical business risks. First, if signage is unclear, users may not understand which facilities they are allowed to use. This could lead to confusion during the short intermission and may cause the unisex stalls to be underused. Second, although the Hybrid Model includes 10 unisex stalls, this capacity may not be sufficient for every event type or audience mix. For example, events with a higher proportion of users who prefer or require unisex facilities may still create long queues for the unisex option. Third, some users may resist changes from the current layout, especially if the purpose of the redesign is not clearly communicated.

There is also a risk that the Hybrid Model improves preference-based fairness but does not reduce waiting time as much as the All Unisex configuration. Therefore, management should not treat the layout change alone as a complete solution. The design should be supported by clear signage, queue monitoring, and staff guidance during busy intermissions, especially when demand is concentrated in the first few minutes.

Future data may also change the recommendation or the expected performance of each configuration. The simulation is based on assumed preference proportions, service-time distributions, and arrival patterns. In practice, different performances may attract different audience profiles, and users’ restroom preferences may change over time. For example, a future event may have more users who prefer unisex access, more users with longer service-time needs, or a different arrival pattern during intermission. Because of this, management should collect real usage data after implementation, including queue lengths, waiting times, facility usage, and user feedback. This would allow the simulation to be updated and help management check whether the Hybrid Model continues to perform well in practice.
...
Our investigation identifies the Hybrid Model (Configuration 3) as the superior solution for the concert hall. The simulation results confirm that the Status Quo (Configuration 1) is operationally fragile and socially inadequate; it fails to absorb demand surges and completely lacks the inclusive design required to address modern patron expectations and complaints.

While the All Unisex (Configuration 2) model offers the lowest raw waiting times through total resource pooling, it suffers from "efficiency leakage", where high-variance service times from Class 2 patrons negatively impact the experience of standard users.

In contrast, the Hybrid Model successfully navigates the "Intermission Challenge" by preserving the high-efficiency throughput of Men’s urinals while introducing 10 universal stalls to ensure flexibility and inclusivity. By consistently achieving the highest Totalitarian (efficiency) and Rawlsian (fairness) equity scores across all stress tests, the Hybrid Model provides a robust, data-driven mandate for facility redesign that balances operational speed with social justice.

## 5.2 Limitations and Future Work

This analysis operates under specific constraints that need further study:

-   Cost Analysis: Excellent business addition. While the Hybrid Model wins on utility, it requires physical renovation (moving walls, structural changes changing signs). Management needs a Net Present Value (NPV) or ROI analysis comparing the cost of renovation against the "cost of customer dissatisfaction."
