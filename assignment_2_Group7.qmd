...
Simulation burst in last 5 minutes while 80% arrive is Men
...
Simulation burst in early 5 minutes while 80% arrive is Men
...
Simulation burst in last 5 minutes while 80% arrive is women
...
generate_stage2_users <- function(rep_id, scenario, arrival_pattern, prob_class2_scenario) {
  ...
  for (i in 1:n_users) {
    ...
    if (gender[i] == "W" & service_class[i] == "class1") {
      service_time[i] <- r_service_gamma(
        1,
        service_mean_women_class1,
        service_sd_women_class1
      )
...
# ============================================================
# Stage 2 Simulation:
# Scenario 1: Female-Dominated Early Rush Stress Test
# ============================================================
...
n_rep <- 500 # Repeat 500 times for stable mean
intermission_length <- 25 
...
```{r}
# ============================================================
# 2. Gamma service-time generator
# ============================================================
# Gamma service time is used because restroom service time must be positive
# and can be right-skewed. The shape and scale are calculated so that
# the generated service time matches the mean and standard deviation.
...
generate_users <- function(
  ...
  if (burst_timing == "early") {
    ...
    stop("This Stage 2 scenario only supports early burst timing.")
  }
  
  # -----------------------------
  # Gender mix
  # -----------------------------
  # The whole audience is female-dominated:
  # 80% women in both burst and non-burst groups.
...
assign_facility <- function(users, config_name, np_unisex_prob = 0.50) {
  ...
  if (config_name == "Hybrid Model") {
    ...
    # No-preference users split 50/50 between gendered and unisex
    ...
    for (cfg in configs) {
      ...
      config_results[[counter]] <- sim %>%
        summarise(
          ...
          pct_completed_25 = mean(completion_time <= intermission_length) * 100,
          queue_failure_rate = mean(completion_time > intermission_length) * 100,
          n_unserved_25 = sum(completion_time > intermission_length),
...
summary_stage2 <- config_results %>%
  ...
  summarise(
    ...
    pct_completed_25 = mean(pct_completed_25),
    queue_failure_rate = mean(queue_failure_rate),
...
decision_stage2 <- summary_stage2 %>%
  ...
  mutate(
    ...
    queue_failure_rank = rank(queue_failure_rate, ties.method = "min"),
    totalitarian_rank = rank(-totalitarian_score, ties.method = "min"),
...
ggplot(plot_wait_scenario, aes(x = config, y = mean)) +
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(
    title = "Mean Waiting Time Under Scenario 1",
    x = "Configuration",
...
```{r}
# ============================================================
# 12. Plot: Queue failure rate
# ============================================================

plot_failure <- config_results %>%
  group_by(scenario, config) %>%
  summarise(
    mean = mean(queue_failure_rate),
    se = sd(queue_failure_rate) / sqrt(n()),
    lower = mean - 1.96 * se,
    upper = mean + 1.96 * se,
    .groups = "drop"
  )

ggplot(plot_failure, aes(x = config, y = mean)) +
  geom_col() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(
    title = "Queue Failure Rate Under Scenario 1",
    x = "Configuration",
    y = "Queue Failure Rate (%)"
  )
```
...
```{r}
# ============================================================
# 13. Plot: Percentage completed within 25 minutes
# ============================================================
...
ggplot(plot_completion, aes(x = config, y = mean)) +
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(
    title = "Percentage Completed Within 25 Minutes Under Scenario 1",
    x = "Configuration",
...
```{r}
# ============================================================
# 14. Plot: Mean waiting time by gender
# ============================================================
...
ggplot(plot_gender_wait, aes(x = config, y = mean, fill = gender)) +
  ...
  labs(
    title = "Mean Waiting Time by Gender Under Scenario 1",
    x = "Configuration",
    y = "Mean Waiting Time (minutes)",
    fill = "Gender"
  )
...
```{r}
# ============================================================
# 15. Plot: Gender waiting-time gap
# ============================================================
...
ggplot(plot_gender_gap, aes(x = config, y = mean)) +
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(
    title = "Gender Waiting-Time Gap Under Scenario 1",
    x = "Configuration",
...
```{r}
# ============================================================
# 16. Plot: Totalitarian equity score
# ============================================================
...
ggplot(plot_totalitarian, aes(x = config, y = mean)) +
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(
    title = "Totalitarian Equity Score Under Scenario 1",
    x = "Configuration",
...
ggplot(plot_rawlsian, aes(x = config, y = mean)) +
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(
    title = "Rawlsian Equity Score Under Scenario 1",
    x = "Configuration",
